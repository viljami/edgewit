...
| `retention`       | No        | Automatically deletes entire partition directories older than this duration based on the partition time. Format: Number followed by a **case-sensitive** unit (`s`, `m`, `h`, `d`, `w`, `M`, `y`). Example: `30d`, `5y`. _Requires `partition` to be set._ |
