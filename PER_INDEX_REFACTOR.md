...
## Step 3: Distributed Search (Scatter-Gather)
- Rewrite `api/search.rs`. Instead of querying a single `IndexReader`, a query to `/indexes/logs/_search` must identify all valid partition directories for the `logs` index.
- Run the Tantivy query against all relevant partitions simultaneously (or sequentially).
- Merge the `TopDocs` (sort by score or time) across the multiple index results.
- Merge Aggregations (using `tantivy::aggregation` tree merging mechanisms) across partitions.
